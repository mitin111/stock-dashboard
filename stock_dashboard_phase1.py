...
with tab5:
    st.set_page_config(page_title="Strategy Engine", layout="wide")
...
intrv = st.selectbox("🕒 Choose Candle Interval", intervals, index=2)
...
for wl in watchlists:
    ...
    for sym in symbols["values"]:
        ...
        if not isinstance(result, list):
            ...
            st.error(f"❌ TPSeries failed for {tsym}: {emsg}")
            continue

        df = pd.DataFrame(result)
        df = df[df["stat"] == "Ok"]
        if df.empty:
            st.warning(f"📭 No valid candle data for {tsym}.")
            continue
